      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-48px)] h-[520px] max-h-[calc(100vh-120px)] bg-white rounded-2xl shadow-2xl border border-accent-gold/20 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-accent-dark to-accent-dark/90 text-white px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-heading text-sm">Assistente Virtual</p>
                ...
                <div className="flex justify-start">
                  <div className="bg-white border border-accent-gold/20 rounded-2xl rounded-bl-md px-4 py-3 shadow-sm">
                    <div className="flex gap-1">
                      ...
                      <span className="w-2 h-2 bg-accent-gold/60 rounded-full animate-bounce" style={{ animationDelay: "0.1s" }} />
                      <span className="w-2 h-2 bg-accent-gold/60 rounded-full animate-bounce" style={{ animationDelay: "0.2s" }} />
                    </div>
                  </div>
                </div>
              )}
            ...
            {messages.length <= 1 && (
              <div className="px-4 py-2 bg-white border-t border-accent-gold/10">
                <p className="text-xs text-text-muted mb-2">Perguntas frequentes:</p>
                <div className="flex flex-wrap gap-2">
                  {["Quais tratamentos vocês oferecem?", "Como agendar consulta?", "Onde fica a clínica?"].map((suggestion) => (
                    <button
                      key={suggestion}
            ...
            {/* Input */}
            <form onSubmit={handleSubmit} className="p-3 bg-white border-t border-accent-gold/10 flex gap-2">
              <input
                type="text"
                value={input}
