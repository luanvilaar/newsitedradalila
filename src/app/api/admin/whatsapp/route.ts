...
import { sendMetaMessage, getMetaConfig } from "@/lib/whatsapp/meta";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Usa Meta Cloud API se configurada, senão fallback para Avisa
    const metaConfig = getMetaConfig();
    const usesMeta = Boolean(metaConfig.token && metaConfig.phoneNumberId);

    let sendResult: { ok: boolean; status: number; error?: string; messageId?: string };
    let providerName: string;

    if (usesMeta) {
      const metaResult = await sendMetaMessage(wa_phone, content.trim());
      sendResult = metaResult;
      providerName = "meta_cloud_api";
    } else {
      const avisaResult = await sendMessageViaAvisa({ phone: wa_phone, text: content.trim() });
      sendResult = avisaResult;
      providerName = "avisaapi";
    }
    ...
    return NextResponse.json({
      ...
      provider: {
        ...
        messageId: (sendResult as { messageId?: string }).messageId,
      },
    });
  } catch (error) {
